...
describe('DK Embark Flows', () => {
  ...
  parameters.forEach((flowUrl) => {
    it(`should get a Danish price quote (${flowUrl})`, () => {
      ...
      // size
      cy.contains('Okay. How many square meters is it?', {
        timeout: 5000,
      }).should('be.visible')
      cy.focused().type(
      ...
      cy.contains('Dansborg Alle 1', { timeout: 1000 })
        .first()
        .click()
      ...
      // co-insured
      cy.contains('Great. How many people will the insurance cover?', {
        timeout: 5000,
      }).click()
      ...
      // birth-date
      cy.contains('And your date of birth?', { timeout: 5000 }).should(
        'be.visible',
      )
      cy.focused().type(`${getRandomBirthDateDK()}{enter}`)

      // email
      cy.contains('Lastly, your email address?', { timeout: 5000 }).should(
        'be.visible',
      )
